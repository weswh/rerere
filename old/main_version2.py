...
from backup.test2 import find_minbox
...
class Feet(object):
    def __init__(self, uid: str, debug: str, measure: str) -> None:
        ...
        self.u_images_path = os.path.join(self.data_path, "undist_images")
        self.u_image_path = os.path.join(self.data_path, "u_image")
        self.u_masks_path = os.path.join(self.data_path, "u_masks")
        self.objpng_path = os.path.join(self.data_path, 'obj2.png')
...
######### Step04 measure
        ...
        # trans=np.array([0,-0.5,0])
        # trans_vertices=vertices+trans
        ...
        # left_width2, left_length2,right_width2, right_length2\
        #     = find_minbox(self.objpng_path)
        # if abs(left_length - right_length) > 5:
        ...
        us_left = foot.calculate_us_size(left)
        us_right = foot.calculate_us_size(right)

        # print('EEEEEE',self.data_path)
        ret0 = {"left": {"length":int(round(left, 0)), 
                             "width": int(round(left_width, 0)), 
                             "farthest": int(round(left_farthest,0)),
                             "size": left_size},
                "right":{"length": int(round(right, 0)), 
                             "width": int(round(right_width, 0)), 
                             "farthest": int(round(right_farthest,0)),
                             "size": right_size}}    


        # ret0 = {"left": {"length":left_length, 
        #                      "width": left_width, 
        #                      "farthest":left_farthest,
        #                      "length2":left_length2, 
        #                      "width2": left_width2, 
        #                      "size": left_size},
        #         "right":{"length": right_length, 
        #                      "width": right_width, 
        #                      "farthest":right_farthest,
        #                      "length2": right_length2, 
        #                      "width2": right_width2, 
        #                      "size": right_size}}   
 
        fio.save_json(os.path.join(self.data_path, "data1.json"), ret0)
