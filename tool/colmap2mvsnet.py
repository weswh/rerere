...
if __name__ == '__main__':
    ...
    for image_id, image in images.items():
        e = np.zeros((4, 4))
        e[:3, :3] = qvec2rotmat(image.qvec)    # 旋转矩阵
        e[:3, 3] = image.tvec               # 平移矩阵
        e[3, 3] = 1
    ...
    for i in range(num_images):
        ...
        for p3d_id in images[i+1].point3D_ids:
            ...
            # zs.append(np.ndarray.item(transformed[2]))
        zs_sorted = sorted(zs)
        # relaxed depth range
    ...
    for i in range(len(images)):
        ...
        view_sel.append([(k, score[i, k]) for k in sorted_score[:5]])  # 只选择得分最高的3个
    print('view_sel[0]\n', view_sel[0], end='\n\n')
